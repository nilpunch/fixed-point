...
namespace Fixed32.Editor
{
	public class FixedPointStatsWindow : EditorWindow
	{
		...
		[MenuItem("Window/Fixed Point/Stats 32")]
		public static void ShowWindow()
		{
			FixedPointStatsWindow wnd = GetWindow<FixedPointStatsWindow>();
			wnd.titleContent = new GUIContent("FP Stats 32");
		}
		...
		private void OnGUI()
		{
			...
			EditorGUILayout.TextField("Max Integer Value", (FMath.Floor(FP.MaxValue).ToInt()).ToString());
			EditorGUILayout.TextField("Min Integer Value", (FMath.Floor(FP.MinValue).ToInt()).ToString());
			EditorGUILayout.TextField("Max Integer To Sqr", (FMath.Sqrt(FP.MaxValue).ToInt()).ToString());
			EditorGUILayout.Space(5f);
