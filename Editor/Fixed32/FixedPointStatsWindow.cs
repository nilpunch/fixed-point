...
namespace Fixed32.Editor
{
	public class FixedPointStatsWindow : EditorWindow
	{
		...
		private void OnGUI()
		{
			...
			EditorGUILayout.TextField("Max Integer Value", (FP.Floor(FP.MaxValue).ToInt()).ToString());
			EditorGUILayout.TextField("Min Integer Value", (FP.Floor(FP.MinValue).ToInt()).ToString());
			EditorGUILayout.TextField("Max Integer To Sqr", (FP.Sqrt(FP.MaxValue).ToInt()).ToString());
			EditorGUILayout.Space(5f);
